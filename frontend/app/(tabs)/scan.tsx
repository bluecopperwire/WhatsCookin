...
import { detectIngredients } from "@/functions/useFetch"; 
...
const scan = () => {
  ...
  const onScan = async () => {
    ...
    try {
      ...
      const data = await detectIngredients(imageUri);
      // data should be like: { ingredients: [{ name: "carrot" }, ...] }
  ...
  return (
    <View className="flex-1 h-56">
      ...
      <Text className="ml-5 mt-5 text-primary text-4xl font-semibold">
        Upload a photo of the
      </Text>
      <Text className="ml-5 mt-3 text-primary text-4xl font-semibold">
        Ingredients
      </Text>

      <Text className="ml-5 mt-5 text-primary text-xl font-normal">
        Please group the ingredients available and
      </Text>
      ...
      <View className=" h-60 mt-10 justify-center items-center">
        <Pressable
          onPress={pickImage} 
          className="bg-[#F7B580] h-60 w-5/6 justify-center items-center rounded-3xl"
        >
          {imageUri ? (
            <Image
              source={{ uri: imageUri }}
